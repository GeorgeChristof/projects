...
# numpy.sin
# Αντίστοιχα ισχύουν για όλες τις βιβλιοθήκες και modules του Python
...
Multiples_Of_Nyquist_Frequensy=1 
...
t = linspace(0, Tmax, int(sample_points+1), endpoint=True) # Από 0 ως Τmax λαμβάνονται κατά Τs sample_points+1 (συμπεριλαμβανομένου και του σημείου για t=0)
...
Multiples_Of_Nyquist_Frequensy=2 # αυτή η εντολή αλλάζει μόνο
our_Sampling_rate=Multiples_Of_Nyquist_Frequensy*Nyquist_Fs
Ts=1/Nyquist_Fs             
our_Ts=1/our_Sampling_rate
Tmax=3*T                    
...
t = linspace(0, Tmax, int(sample_points+1), endpoint=True) 
